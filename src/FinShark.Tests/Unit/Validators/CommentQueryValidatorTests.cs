...
public class GetAllCommentsQueryValidatorTests
{
    ...
    {
        var query = new GetAllCommentsQuery(new CommentQueryRequestDto { PageNumber = 1, PageSize = 20 });
    ...
    {
        var query = new GetAllCommentsQuery(new CommentQueryRequestDto { PageNumber = 1, PageSize = 0 });
    ...
    {
        var query = new GetAllCommentsQuery(new CommentQueryRequestDto
        {
            PageNumber = 1,
            PageSize = 20,
            SortBy = CommentSortBy.Created,
...
public class GetCommentsByStockIdQueryValidatorTests
{
    ...
    public async Task ValidateAsync_WithValidParameters_ShouldPass()
    {
        var query = new GetCommentsByStockIdQuery(1, new CommentQueryRequestDto { PageNumber = 1, PageSize = 20 });
    ...
    {
        var query = new GetCommentsByStockIdQuery(0, new CommentQueryRequestDto { PageNumber = 1, PageSize = 20 });
    ...
    {
        var query = new GetCommentsByStockIdQuery(1, new CommentQueryRequestDto
        {
            PageNumber = 1,
            PageSize = 20,
            SortBy = (CommentSortBy)99,
    ...
    {
        var query = new GetCommentsByStockIdQuery(1, new CommentQueryRequestDto { MinRating = 5, MaxRating = 2 });
        var result = await _validator.ValidateAsync(query);
        result.IsValid.Should().BeFalse();
    }
}
